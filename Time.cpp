...
{ // copy constructor
    ...
    {
        pText = new char[9];
        strncpy(pText, Original.pText, 9);
    }
    else
    {
        pText = nullptr;
...
{
    if (h < 0 || h > 23)
        throw std::runtime_error("Wrong hours");
    Hours = h;
}
...
{
    if (m < 0 || m > 59)
        throw std::runtime_error("Wrong minutes");
    Mins = m;
}
...
{
    if (s < 0 || s > 60)
        throw std::runtime_error("Wrong seconds");
    Secs = s;
}
...
Time::operator char *() const
{
    ...
    snprintf(pText, 9, "%02d:%02d:%02d", Hours, Mins, Secs);
    return pText;
}
